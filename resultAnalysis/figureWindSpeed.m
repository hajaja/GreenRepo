...
for n = 1:numExampleLocations
    ...
    tracesSpeed(:, n) = trace;
    [x, y] = funPDF(trace, 'hourly');
    tracesPDF(:, n) = y;
    tracesPDFX(:, n) = x;
end
traceSpeedMean = mean(tracesSpeed, 2);
[x, tracePDFMean] = funPDF(traceSpeedMean, 'hourly');
%% speed
figure1 = figure;
...
box on;
for n = 1:numExampleLocations
    plot(xOptimal, tracesSpeed(:, n), 'color',colorVec(n), 'markersize',markersize,'linewidth',linewidth);
end
plot(xOptimal, traceSpeedMean, 'color', 'b', 'marker', 'x', 'markersize',markersize,'linewidth',linewidth);
hold off;
xlabel('Time (hour)');
ylabel(unit);
xlim([0, max(xOptimal)]);
...
plot(x, tracePDFMean, 'color', 'b', 'marker', 'x', 'markersize',markersize,'linewidth',linewidth);
hold off;
